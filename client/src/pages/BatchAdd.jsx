    ...
    <div className={`bg-white rounded-xl shadow-sm border ${error ? 'border-red-300' : 'border-slate-200'} p-4`}>
      ...
      {item.mode === 'existing' && (
        <div className="space-y-3">
          ...
          {/* Quantity - Only show when product is selected */}
          {item.selectedProduct && (
          ...
          {/* Price Update Toggle - Hidden by default */}
          {item.selectedProduct && !item.updatePrice && (
            <button
              type="button"
              onClick={() => onPriceUpdateToggle(index, true)}
              className="text-xs text-farm-600 hover:text-farm-700 underline"
            >
              Update harga produk
            </button>
          )}

          {/* Price Update Option - Only show when enabled */}
          {item.selectedProduct && item.updatePrice && (
            <div className="flex items-center gap-2">
              <label className="text-sm text-slate-600">Harga baru:</label>
              <input
                type="number"
                min="0"
                step="100"
                ...
                placeholder="Rp 0"
                className="flex-1 px-3 py-1.5 border border-farm-500 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-farm-500"
                autoFocus
              />
              <button
                type="button"
                onClick={() => onPriceUpdateToggle(index, false)}
